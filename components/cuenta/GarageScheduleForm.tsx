          ...
          >
            ...
            {day.isOpen ? (
              <div className="flex items-center gap-2 flex-1">
                <Clock className="h-3.5 w-3.5 text-gartify-gray/60 shrink-0" />
                <select
                  value={day.openTime}
                  onChange={(e) => updateDay(dow, { openTime: e.target.value })}
                  className="h-8 rounded-lg border border-gray-200 bg-white px-2 text-sm text-gartify-blue font-medium focus:outline-none focus:ring-2 focus:ring-gartify-hero/30"
                >
                  {TIME_OPTIONS.map((t) => (
